...
import { Unit, Trigger, MapPlayer } from "w3ts/index";
...
export function enableLastHitOnCreepTrigger() {
  ...
  const checkEnemyIsNearbyAndDyingUnitIsCreepAndKillerisNotCreep = () => {
    ...
    ForGroupBJ(GetUnitsInRangeOfLocAll(600.00, GetUnitLoc(GetDyingUnit())), () => {
  ...
  t.addAction(() => showMessageOverUnit(Unit.fromHandle(GetDyingUnit()), Unit.fromHandle(GetKillingUnit()).owner, "•", 13, MapPlayer.fromLocal().isObserver()))
}
